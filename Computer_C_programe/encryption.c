...
char KEY[KEYSIZE] = {5, 10, 15, 20, 25, 30, 35, 40, 45, 50};
char IV[KEYSIZE] = {219, 128, 87, 73, 139, 125, 159, 132, 91, 193};

void encrypt(unsigned char *msg, int msglen){
	for(int i = 0; i < msglen; i++){
		msg[i] = (unsigned char)(msg[i] ^ KEY[i % 9]);
	}
}

void decrypt(unsigned char *msg, int msglen){
	for(int i = 0; i < msglen; i++){
		msg[i] = (unsigned char)(msg[i] ^ KEY[i % 9]);
	}
}
...
void CBC_decrypt(unsigned char *msg, int msglen){
	int block_idx = 0;
	int block_size = 0;
	char curr_block_vector[KEYSIZE] = {};
	char last_block_vector[KEYSIZE] = {};
